    ...
    <div className="min-h-screen bg-[#080e1a] text-white overflow-x-hidden">
      ...
      <section ref={heroRef} className="relative flex items-center overflow-hidden"
        ...
        <motion.div style={{ opacity: heroOpacity, pointerEvents: 'auto' }}
          className="hidden lg:grid max-w-7xl mx-auto px-6 w-full grid-cols-2 gap-6 items-center"
          ...
          <motion.div initial={{ opacity:0, x:-50 }} animate={{ opacity:1, x:0 }}
            ...
            <h1 className="font-display font-900 leading-[1.05] mb-3">
              <motion.span initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.3 }}
                className="block text-white text-5xl xl:text-6xl">Industrial</motion.span>
              <motion.span initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.4 }}
                className="block gradient-text-yellow text-5xl xl:text-6xl neon-yellow">Air Compressor</motion.span>
              <motion.span initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.5 }}
                className="block text-white text-5xl xl:text-6xl">Optimizer</motion.span>
            </h1>
        ...
        {/* ══ MOBILE LAYOUT (< lg): stacked, text on top ══ */}
        <motion.div style={{ opacity: heroOpacity, pointerEvents: 'auto' }}
          className="lg:hidden w-full h-full flex flex-col px-4 pt-20 pb-4 overflow-y-auto">
          ...
          <div className="flex-shrink-0">
            <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.2 }}
              ...
              style={{ border:'1px solid rgba(250,204,21,0.3)', background:'rgba(250,204,21,0.06)' }}>
              <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 animate-pulse" />
              <span className="text-yellow-400 text-xs font-mono">AI-Powered Industrial Solution</span>
            </motion.div>
            <h1 className="font-display font-900 leading-tight mb-2">
              <span className="block text-white text-3xl sm:text-4xl">Industrial</span>
              <span className="block gradient-text-yellow text-3xl sm:text-4xl neon-yellow">Air Compressor</span>
              <span className="block text-white text-3xl sm:text-4xl">Optimizer</span>
            </h1>
            <p className="text-slate-400 text-xs leading-relaxed mb-3 max-w-sm">
              Upload compressor data — our AI pipeline finds
              <span className="text-yellow-400"> optimal operating parameters</span> for max efficiency.
            </p>
            <div className="flex gap-2 mb-3">
              <button onClick={handleStart}
                className="btn-primary flex items-center gap-1.5 text-xs px-4 py-2.5">
                Start Optimizing <ChevronRight size={14} />
              </button>
              <button onClick={() => navigate('/tutorial')}
                className="btn-ghost-yellow flex items-center gap-1.5 text-xs px-4 py-2.5">
                Tutorial <ArrowRight size={12} />
              </button>
            </div>
            {/* Stats row */}
            <div className="grid grid-cols-4 gap-2 pb-2 border-b border-white/5">
              {[
                ...
                { val:'20%+',  label:'Saving'   },
                { val:'3-Step',label:'Pipeline'  },
                { val:'∞',     label:'Units'     },
              ].map(({ val, label }) => (
                <div key={label} className="text-center">
                  <div className="font-display font-900 text-sm text-yellow-400">{val}</div>
          ...
          {/* Bottom: 3D Model — fills remaining space */}
          <div className="flex-1 min-h-0 mt-2 rounded-2xl overflow-hidden relative"
            style={{
              background: 'radial-gradient(ellipse at center, rgba(8,20,40,0.6) 0%, rgba(4,10,20,0.95) 100%)',
              border: '1px solid rgba(0,212,255,0.1)',
            }}>
